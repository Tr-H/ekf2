...
void get_dcm_from_euler(Eigen::Matrix3d &dcm, const Eigen::Vector3d &e) {
    double phi = e[0];
    double theta = e[1];
    double psi = e[2];
    dcm(0, 0) = cosf(theta) * cosf(psi); 
    dcm(0, 1) = - cosf(phi) * sinf(psi) + sinf(phi) * sinf(theta) * cosf(psi); 
    dcm(0, 2) = sinf(phi) * sinf(psi) + cosf(phi) * sinf(theta) * cosf(psi);
    dcm(1, 0) = cosf(theta) * sinf(psi);
    dcm(1, 1) = cosf(phi) * cosf(psi) + sinf(phi) * sinf(theta) * sinf(psi);
    dcm(1, 2) = - sinf(phi) * cosf(psi) + cosf(phi) * sinf(theta) * sinf(psi);
    dcm(2, 0) = - sinf(theta);
    dcm(2, 1) = sinf(phi) * cosf(theta);
    dcm(2, 2) = cosf(phi) * cosf(theta);
}

void get_q_from_euler(Eigen::Quaterniond &q, const Eigen::Vector3d &e) {
    Eigen::Matrix3d temp_R;
    get_dcm_from_euler(temp_R, e);
    get_q_from_dcm(q, temp_R);
}
#endif